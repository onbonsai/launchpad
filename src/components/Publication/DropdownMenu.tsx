...
import { SmartMedia, SET_FEATURED_ADMINS, setFeatured, ELIZA_API_URL } from "@src/services/madfi/studio";
import { CreditBalance } from "@src/hooks/useGetCredits";
...
export default ({
  ...
  const showFeaturedToggle = useMemo(
    () => address && SET_FEATURED_ADMINS.includes(address?.toLowerCase()) && mediaUrl?.includes("onbons.ai"),
    [mediaUrl, address],
  );
