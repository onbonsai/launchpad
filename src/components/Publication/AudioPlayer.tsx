...
export function AudioPlayer({ profile, url, cover, theme }) {
  ...
  return (
    <div>
      <ReactPlayer url={url} playing={isPlaying} className={playerStyle} />
      <div className={controlsStyle(audioPlayerBg)} onClick={updateIsPlaying}>
        <div className={coverImageContainerStyle}>
          {cover ? <img src={cover} className={audioImaveStyle} /> : <div />}
        </div>
        ...
        <div className={playIconWrapperStyle}>
          <div className={playIconContainerStyle(playIconBgColor)}>{isPlaying ? <PauseIcon /> : <PlayIcon />}</div>
        </div>
      </div>
    </div>
  );
