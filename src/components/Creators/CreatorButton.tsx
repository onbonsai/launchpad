...
const CreatorButton = (props: CreatorButtonProps) => {
  ...
  return (
    <div
      onClick={onClick}
      className={clsx(
        "flex flex-row gap-[6px] bg-card rounded-lg",
        onClick ? "hover" : "",
        image ? "p-[2px] pr-[8px]" : "px-2 py-1",
      )}
    >
      {image && <img src={image} alt="user" className="w-5 h-5 rounded-md" />}
      <p className={clsx("text-sm font-medium tracking-[-0.02em]", image ? "" : "leading-4")}>{handle}</p>
    </div>
  );
};
