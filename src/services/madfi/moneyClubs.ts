...
const CLUB_BALANCE = gql`
  query ClubChips($trader: Bytes!, $club: Bytes!) {
    clubChips(where: { trader: $trader, club: $club }) {
      id
      amount
      club {
        id
...
const GET_TRADER = gql`
  query GetTrader($id: ID!, $isBuy: Boolean!, $createdAt_gt: Int!) {
    trader(id: $id) {
      trades(
        where: { isBuy: $isBuy, createdAt_gt: $createdAt_gt }
        orderBy: txPrice
        orderDirection: desc
        first: 100
      ) {
...
export const ADMIN_WALLET = "0x21aF1185734D213D45C6236146fb81E2b0E8b821";
...
export const getRegisteredClubByTokenAddress = async (tokenAddress: `0x${string}`, chain = "lens") => {
  try {
    ...
    return data.clubs?.length ? { ...data.clubs[0], chain } : null;
  } catch (error) {
    console.log(error);
  }
};
...
export const getRegisteredClubInfoByAddress = async (tokenAddress: any, chain = "lens") => {
  ...
  const {
    data: { clubs },
  } = await client.query({ query: REGISTERED_CLUB_INFO_BY_ADDRESS, variables: { tokenAddress } });
...
export const searchClubs = async (query: string, chain = "lens") => {
  ...
  const {
    data: { clubs },
  } = await client.query({ query: SEARCH_CLUBS, variables: { query } });
  return clubs?.map((club: any) => {
...
): Promise<{ address: `0x${string}`; balance: bigint }[]> => {
  try {
    ...
    return (
      response.data?.result?.tokenBalances?.map(
        ({ contractAddress, tokenBalance }: { contractAddress: `0x${string}`; tokenBalance: string }) => ({
...
export const getHoldings = async (
  ...
  const {
    ...
    query: HOLDINGS_PAGINATED,
    variables: { trader: account.toLowerCase(), startOfDayUTC, skip },
  });
  ...
  const tokenBalances = completeTokens?.length ? await getTokenBalances(account, completeTokens) : [];
  const balanceMap = new Map(tokenBalances.map((b: any) => [b.address.toLowerCase(), b.balance]));

  const holdings = await Promise.all(
    clubChips?.map(async (chips: any) => {
      ...
      const amount = complete
        ...
        : chips.club?.v2
        ? formatEther(BigInt(chips.amount))
        : formatUnits(BigInt(chips.amount), 6);
      ...
      const balance = false // complete && IS_PRODUCTION
        ...
        : Number.parseFloat(amount) *
          Number.parseFloat(formatUnits(chips.club?.currentPrice || 0n, chain === "base" ? USDC_DECIMALS : DECIMALS));
...
export const getClubHoldings = async (
  ...
  const {
    data: { clubChips },
  } = await client.query({ query: CLUB_HOLDINGS_PAGINATED, variables: { club: id, skip } });
...
export const getFeaturedClubs = async (chain = "lens"): Promise<any[]> => {
  ...
  if (clubs?.length) {
    ...
    const {
      data: { clubs: _clubs },
    } = await subgraphClient(chain).query({ query: REGISTERED_CLUBS_BY_ID, variables: { ids } });

    try {
      // TODO: fetch for other strategies (ie orb_club, farcaster)
      const publications = await (lensClient as any).publication.fetchAll({
        where: {
          publicationIds: clubs
            .filter(
              ({ strategy, postId }) =>
                strategy === "lens" && !!postId && typeof postId === "string" && postId.trim() !== "",
            )
            .map(({ postId }) => postId),
        },
      });
...
export const getRegisteredClubs = async (
  ...
  if (data?.clubs?.length) {
    ...
    try {
      const lensTokens = clubs.filter(
        ({ strategy, postId }) => strategy === "lens" && !!postId && typeof postId === "string" && postId.trim() !== "",
      );
      const publications: Post[] = (await getPostsBatched(
        lensTokens.map(({ postId }) => postId),
      )) as unknown[] as Post[];
      const gPublications = groupBy(publications || [], "slug");
      const groupedClubs = groupBy(clubs || [], chain === "base" ? "clubId" : "tokenAddress");
      const responseClubs = data?.clubs
        .map((_club: any) => {
          const marketCap =
            BigInt(_club.supply) <= FLAT_THRESHOLD && _club.v2
              ? BigInt(_club.liquidity)
              : formatUnits(
                  BigInt(_club.liquidityReleasedAt ? parseUnits("1000000000", DECIMALS) : _club.supply) *
                    BigInt(_club.currentPrice.toString()),
                  DECIMALS,
                ).split(".")[0];
          const club =
            chain === "base"
              ? groupedClubs[_club.clubId.toString()]
                ? groupedClubs[_club.clubId.toString()][0]
                : undefined
              : groupedClubs[_club.tokenAddress]
              ? groupedClubs[_club.tokenAddress][0]
              : undefined;
          if (club?.hidden) return; // db forced hide
          let { name, symbol, uri: image } = _club;
          if (_club.tokenInfo && (!name || !symbol || !image)) {
          ...
          if (club) {
            ...
            return {
              ...
              marketCap,
              token,
              handle: publication?.author?.username?.localName || _club.creator,
            };
          } else {
            // not created on our app
            return {
...
export const getBalance = async (
  ...
  const {
    data: { clubChips },
  } = await client.query({ query: CLUB_BALANCE, variables: { trader: account.toLowerCase(), club: id } });
    ...
    buyPrice = !!supply
      ? await client.readContract({
          ...
          args: [
                  ...
                  pricingTier?.flatThreshold,
                  pricingTier?.targetPriceMultiplier,
                ],
        })
      : await client.readContract({
...
function calculateTokensForUSDC(
  ...
  function getPrice(
    ...
    amount: bigint,
    initialPrice: bigint,
    targetPriceMultiplier: bigint,
    flatThreshold: bigint,
  ): bigint {
    if (supply < flatThreshold) {
      ...
      return (
        (flatAmount * initialPrice) / BigInt(10 ** decimals) +
        getPrice(flatThreshold, curveAmount, initialPrice, targetPriceMultiplier, flatThreshold)
      );
    }
  ...
  function calculateDeltaArea(startSupply: bigint, endSupply: bigint, slope: bigint, initialPrice: bigint): bigint {
...
export const getBuyAmount = async (
  ...
  const rawBuyAmount =
    options?.initialPrice && options?.targetPriceMultiplier && options?.flatThreshold
      ? calculateTokensForUSDC(
          spendAfterFees,
          currentSupply,
          BigInt(options.initialPrice!),
          BigInt(options.targetPriceMultiplier!),
          BigInt(options.flatThreshold!),
        )
      : calculateTokensForUSDC(
          spendAfterFees,
          currentSupply,
  ...
  if (BigInt(buyAmount || 0n) + BigInt(currentSupply) >= MAX_MINTABLE_SUPPLY) {
    ...
    effectiveSpend = (await getBuyPrice(account, "0", adjustedAmount, formatUnits(currentSupply, DECIMALS)))
      .buyPriceAfterFees;
  }
...
): { valuePct: number; positive?: boolean; neutral?: boolean } => {
...
export const releaseLiquidity = async (
  clubId: string,
  ...
  const {
    data: { token, hash },
  } = await axios.post(`/api/clubs/release-liquidity`, {
    clubId,
    tokenPrice,
    chain,
    clubCreator,
  ...
  await fetch("/api/clubs/update", {
    ...
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      updateRecord: { clubId: parseInt(BigInt(clubId).toString()), liquidityReleasedTxHash: hash, chain },
    }),
  });
